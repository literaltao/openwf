...
double Cellset::dist(int inda, int indb) {
	...
	if (d["ATTACK_NAME"] == "Pa-CUMUL.py") {
		...
		double* featb = fetch_pool(indb);
		if (d["DIST_FEAT"] == "0") {
			printf("Pa-FeaturesSVM.py requires DIST_FEAT=1");
			exit(-1);
		}
		int minlen = sizea;
		if (sizea > sizeb) minlen = sizeb;
	...
	if (d["ATTACK_NAME"] == "Wa-kNN.py") {
		...
		double* featb = fetch_pool(indb);
		if (d["DIST_FEAT"] == "0") {
			printf("kNN.py requires DIST_FEAT=1");
			exit(-1);
		}
...
//
	...
	if (CORE == 0) {
		...
		int jobsent = 0;
		int jobdone = 0;
		int jobtotal = OPEN_INSTNUM * (CLOSED_INSTNUM * CLOSED_SITENUM) + (CLOSED_INSTNUM * CLOSED_SITENUM - 1) * (CLOSED_INSTNUM * CLOSED_SITENUM)/2;
		...
		for (talking_core = 1; talking_core < CORE_TOTAL; talking_core++) {
			if (jobsent < jobtotal) {
				vector<int> jobxy = job_to_jobxy(jobsent);
