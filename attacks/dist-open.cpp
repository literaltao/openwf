...
//dist.cpp has a size issue - we can't load all 100,000 into memory
//therefore, we only do the closedxclosed set (100x200, divided into 4) in dist.cpp 
//here, we do the openxclosed and openxopen sets; 
//we don't have to do closedxopen because we can just parse that from openxclosed

//openxclosed:
//depend on the modulo 4 of the file name, choose one of the four portions of the 100x200 closed set
//only load ONE of the open portions
...
int CLOSED_SITENUM, CLOSED_INSTNUM, OPEN_INSTNUM;
int CLOSED_FOLD_TOTAL;
int FOLD_SIZE;
map<string, string> d;
...
Cellset::Cellset(int is_parse_cells){
	...
	printf("Loading open: %d\n", FOLD_SIZE);

	if (CORE > 0) {
		for(int i = FOLD_SIZE * (CORE - 1); i < FOLD_SIZE * CORE; i++){
			...
			pool_names.push_back(strfname);
			count += 1;
			if ((count * 10) / FOLD_SIZE > progcount) {
...
//
	...
	if (CORE_TOTAL % 2 == 0 or CORE_TOTAL % 5 == 0) {
		printf("Remember to add 1 to CORE_TOTAL (=%d)!\n", CORE_TOTAL);
	}
	...
	FOLD_SIZE = OPEN_INSTNUM/ (CORE_TOTAL - 1);
	...
	if (CORE == 0) {
		FILE * fp = NULL;
		string fname;
		fname = d["OUTPUT_LOC"] + d["ATTACK_NAME"] + "-open.predist";
		...
		//start every other core
		for (talking_core = 1; talking_core < CORE_TOTAL; talking_core++) {
			MPI_Send(ignore, 1, MPI_INT, talking_core, tag, MPI_COMM_WORLD);
		...
		clock_t t1;
		int * recvcount = new int[CORE_TOTAL];
		for (int i = 0; i < CORE_TOTAL; i++) {
			recvcount[i] = 0;
		}
		int expcount = (CLOSED_INSTNUM * CLOSED_SITENUM / CLOSED_FOLD_TOTAL + FOLD_SIZE) * FOLD_SIZE;
		int coredone = 1;
		while (coredone < CORE_TOTAL) {
			MPI_Status status;
			int talking_core;
			...
			recvcount[talking_core] += 1;
			if (recvcount[talking_core] == expcount) coredone += 1;
			if (recvcount[talking_core] < expcount) {
				//printf("%d tells %d to do job %d %d with %d %d %d\n", 0, talking_core, my_job[0], my_job[1], jobsent, jobdone, jobtotal);
				MPI_Send(ignore, 1, MPI_INT, talking_core, tag, MPI_COMM_WORLD);
				MPI_Irecv(buf[talking_core], 3, MPI_DOUBLE, talking_core, tag, MPI_COMM_WORLD, &request[talking_core]);
			}
			if (recvcount[talking_core] % 1000 == 0) {
				t1 = clock();
				printf("%f %d Job %d out of %d done\n", ((float)t1)/CLOCKS_PER_SEC, talking_core, recvcount[talking_core], expcount);
			}
		}
		fclose(fp);
	}
		...
		for (int i = 0; i < FOLD_SIZE; i++) {
			sinste[0] = i + CLOSED_SITENUM * CLOSED_INSTNUM; //this one is used internally for dist
			int sendsinste = (CORE - 1) * FOLD_SIZE + i + CLOSED_SITENUM * CLOSED_INSTNUM; //this one is sent back to the master
			int my_portion = i % CLOSED_FOLD_TOTAL;
			for (int s = 0; s < CLOSED_SITENUM; s++) {
				for (int m = my_portion * (CLOSED_INSTNUM/CLOSED_FOLD_TOTAL); m < (my_portion+1) * (CLOSED_INSTNUM/CLOSED_FOLD_TOTAL); m++) {
					sinste[1] = s * CLOSED_INSTNUM + m;
					double my_dist = my_cellset.dist(sinste[0], sinste[1]);
					double reply[3] = {(double)sendsinste, (double)sinste[1], my_dist};
					//just so we're not sending too quickly, we wait for a message to ignore
					MPI_Status status;
					MPI_Recv(ignore, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
					MPI_Send(reply, 3, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);
				}
			}
			for (int j = 0; j < FOLD_SIZE; j++) {
				sinste[1] = j + CLOSED_SITENUM * CLOSED_INSTNUM; //this one is used internally for dist
				int sendsinste1 = (CORE - 1) * FOLD_SIZE + j + CLOSED_SITENUM * CLOSED_INSTNUM; //this one is sent back to the master
				double my_dist = my_cellset.dist(sinste[0], sinste[1]);
				double reply[3] = {(double)sendsinste, (double)sendsinste1, my_dist};
				//just so we're not sending too quickly, we wait for a message to ignore
				MPI_Status status;
				MPI_Recv(ignore, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
				MPI_Send(reply, 3, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);
