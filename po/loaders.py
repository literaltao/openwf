...
def get_dillname(d):
    if "CLOSED_SITESTART" in list(d.keys()):
    ...
    if "OPEN_INSTSTART" in list(d.keys()):
...
def load_dill(fname, d):
    ...
    if fname != get_dillname(dillset):
        print(
            "load_dill warning: expected input {}, got {}".format(
                get_dillname(d), fname
            )
        )
...
def write_dill(fname, data, names, d):
    ...
    if fname != get_dillname(d):
        print(
            "write_dill warning: expected output {}, got {}".format(
                get_dillname(d), fname
            )
        )
    ...
    for site1 in range(startsite, len(cellnames)):
        for inst1 in range(startinst, len(cellnames[site1])):
            if count != 0:
                ...
                exp_time = 0
            print(
                "write_dist {}/{}-{} time left {}".format(
                    site1, len(cellnames), inst1, int(exp_time)
                )
            )
            count += 1
...
def get_list(d):
    ...
    if "TRAIN_CINSTNUM" in list(d.keys()):
    ...
    if d["FOLD_MODE"] == 3:
        for s in range(cstart, cend):
            for i in range(0, iend):
                sname = d["DATA_LOC"] + str(s) + "-" + str(i) + "." + d["DATA_TYPE"]
                if i >= d["CLOSED_INSTNUM"] / foldtotal * d["FOLD_NUM"] and i < d[
                    "CLOSED_INSTNUM"
                ] / foldtotal * (d["FOLD_NUM"] + 1):
                    test_list[s].append(sname)
                else:
                    train_list[s].append(sname)
        for s in range(ostart, oend):
            sname = d["DATA_LOC"] + str(s) + "." + d["DATA_TYPE"]
            if (
                s >= (oend - ostart) / foldtotal * d["FOLD_NUM"] + ostart
                and s < (oend - ostart) / foldtotal * (d["FOLD_NUM"] + 1) + ostart
            ):
                test_list[-1].append(sname)
            else:
                train_list[-1].append(sname)
    ...
    if d["FOLD_MODE"] == 4:
        ...
        for s in range(cstart, cend):
            for i in range(0, d["CLOSED_INSTNUM"]):
                sname = d["DATA_LOC"] + str(s) + "-" + str(i) + "." + d["DATA_TYPE"]
                if i >= d["CLOSED_INSTNUM"] / foldtotal * trainfoldnum and i < d[
                    "CLOSED_INSTNUM"
                ] / foldtotal * (trainfoldnum + 1):
                    train_list[s].append(sname)
        for s in range(ostart, oend):
            sname = d["DATA_LOC"] + str(s) + "." + d["DATA_TYPE"]
            if (
                s >= (oend - ostart) / foldtotal * trainfoldnum + ostart
                and s < (oend - ostart) / foldtotal * (trainfoldnum + 1) + ostart
            ):
                train_list[-1].append(sname)
